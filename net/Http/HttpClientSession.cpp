...
void HttpClientSession::readHandle()
{
	//testDealWebSocket();
...
void HttpClientSession::dealRestfulReq()
{
	char strbuf[1024];
	ByteBuffer response;
	response.init(4096);
...
void HttpClientSession::dealFileQeq()
{
...
std::string HttpClientSession::getCurrentTimeInGMT()
{
	// 获取当前时间并转换为时间结构
