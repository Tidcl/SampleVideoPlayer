...
RtpConnection::RtpConnection(std::weak_ptr<Channel> rtsp_connection)
...
bool RtpConnection::SetupRtpOverTcp(MediaChannelId channel_id, uint16_t rtp_channel, uint16_t rtcp_channel)
{
	...
	rtpfd_[channel_id] = conn->fd();
	rtcpfd_[channel_id] = conn->fd();
...
bool RtpConnection::SetupRtpOverUdp(MediaChannelId channel_id, uint16_t rtp_port, uint16_t rtcp_port)
{
	...
	if(SocketUtil::GetPeerAddr(conn->fd(), &peer_addr_) < 0) {
		return false;
	}
...
string RtpConnection::GetRtpInfo(const std::string& rtsp_url)
{
	...
	auto time_point = std::chrono::time_point_cast<chrono::milliseconds>(std::chrono::steady_clock::now());
...
int RtpConnection::SendRtpOverTcp(MediaChannelId channel_id, RtpPacket pkt)
{
	...
	conn->writeByteBuffer().append((const char*)rtpPktPtr, (size_t)pkt.size);
	return pkt.size;
}
